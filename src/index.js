...
const generarInterfazPubli = (arr) => {
  const containerGalery = document.querySelector('#galery-trabajos_ig');
  let fragment = document.createDocumentFragment();
  ...
  arr.map((item, i) => {
    ...
    for (const itemPubliMulti of item.multimedia) {
      ...
      if (!itemPubliMulti.includes('webm')) {
        ...
        img.setAttribute('alt', item.trabajosP);
    ...
    carouselInner.children[0].children[1].children[0].textContent =
      item.trabajosP;
    carouselInner.children[0].children[1].children[1].textContent =
      item.descripcion;
